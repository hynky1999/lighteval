...
from ..tasks.nli.xnli import AfriXNLITask, XNLITask, XNLI2Task
...
_MC_TASKS = [
    ...
    *[OpenAIMMLUTask(subset=subset) for subset in get_args(MMLU_SUBSET)],
    AfriXNLITask(),
...
TASKS_GROUPS = {
    ...
    "xnli": tasks_to_string(
        [XNLITask(lang="sw", version=version) for version in (1, 2)]
    ...
    "m3exam": tasks_to_string(
        [M3ExamTask(lang="sw", version=version) for version in (2,)]
    ),
